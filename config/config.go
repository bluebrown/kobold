...
func (cfg *Config) Apply(ctx context.Context, q *store.Queries) error {
	...
	for _, c := range cfg.Channels {
		// ch := store.ChannelPutParams{Name: c.Name, DecoderName: dbutil.NullString{String: c.Decoder, Valid: c.Decoder != ""}}
		ch := store.ChannelPutParams{Name: c.Name, DecoderName: null.NewString(c.Decoder, c.Decoder != "")}
	...
	for _, p := range cfg.Pipelines {
		if err := q.PipelinePut(ctx, store.PipelinePutParams{
			Name:         p.Name,
			RepoUri:      p.RepoURI,
		...
		for _, c := range p.Channels {
			if err := q.SubscriptionPut(ctx, store.SubscriptionPutParams{
				PipelineName: p.Name,
				ChannelName:  c,
			}); err != nil {
